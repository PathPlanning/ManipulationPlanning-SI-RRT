...
def process_one_task(test_dir_path:str ,path_to_strrt_config_json:str,seed,test_count_number,full_test_number, planner_bin_path:str,planner_prefix:str,planner_logs_prefix:str) -> None:
    ...
    for robot in init_scene_parsed_data["robots"]:
        obs_robot = copy.deepcopy(robot)
        obs_robot["type"] = "dynamic_robot"
        obs_robot["trajectory"] = [{"time": -0.1,
                            "robot_angles": obs_robot['start_configuration']}]
        obs_robot["urdf_file_path"] = 'Blender/robots/xarm6/xarm6cyl.urdf' #r["robot_urdf"]
        obs_robot["robot_urdf"] = 'Blender/robots/xarm6/xarm6cyl.urdf' #r["robot_urdf"]
        obstacles_dict[robot['name']] = obs_robot
        
    
    start_frame_for_robot = {}
    for robot in init_scene_parsed_data["robots"]:
        start_frame_for_robot[robot['name']]=0.0
        
    for goal_number in range(len(init_scene_parsed_data["robots"][0]['end_configuration'])):
        for robot in init_scene_parsed_data["robots"]:
            if goal_number == 0:
            ...
            goal_configuration = robot['end_configuration'][goal_number]
            need_to_go = True
            attempt=1
            start_time = start_frame_for_robot[robot['name']]
            obstacles = obstacles_dict.values()
            strrtlogs = []
            while need_to_go:
                frame_count = int(500*attempt**1.3) # heuristic
                
                scene_task_filepath = create_one_agent_scene_task_json( 
                start_configuration, goal_configuration, goal_number, start_time, obstacles,
            ...
            start_frame_for_robot[robot['name']] = end_time 

            obstacles_dict[robot['name']]['trajectory'].extend(strrtlogs["final_planner_data"]["final_path"])

            # print(test_dir_path)
            for i in range(len(obstacles_dict[robot['name']]['trajectory'])-1):
                if not obstacles_dict[robot['name']]['trajectory'][i]["time"]-0.0001 <= obstacles_dict[robot['name']]['trajectory'][i+1]["time"]:
                    print(obstacles_dict[robot['name']]['trajectory'][i]["time"],obstacles_dict[robot['name']]['trajectory'][i+1]["time"])
                    print(test_dir_path, test_count_number,robot["name"])
                    print(obstacles_dict[robot['name']]['trajectory'])
                assert(obstacles_dict[robot['name']]['trajectory'][i]["time"]-0.0001 <= obstacles_dict[robot['name']]['trajectory'][i+1]["time"])
