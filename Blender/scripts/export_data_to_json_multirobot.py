...
joint_bounds):
    ...
    result = {"name": name,
            "type":"dynamic_robot",
            "robot_urdf":"./Blender/robots/xarm6/xarm6vis.urdf",# robot_urdf_path, TODO: Remove hardcode
...
def add_plannable_robot(robots,name,start_pos,goal_pos ,robot_urdf_path ):
    movable_joints, joint_bounds = parse_urdf(robot_urdf_path)
    start_configuration = get_robot_angles(bpy.data.objects[start_pos],movable_joints, joint_bounds)
    end_configuration = get_robot_angles(bpy.data.objects[goal_pos],movable_joints, joint_bounds)
    print(joint_bounds)
    ...
    bpy.context.scene.frame_set(0)
    obj = bpy.data.objects[start_pos]
    robot_pos = list(obj.matrix_world.to_translation())
...
def main():
    ...
    ### ADD BOXES
    obstacle_names = ['floor','table','table_leg_1','table_leg_2','table_leg_3','table_leg_4','cup_1','cup_2']
    ...
    robots = []
    add_plannable_robot(robots,name = 'robot1', start_pos='robot_start_pos',goal_pos = "robot_goal_pos",robot_urdf_path = robot_urdf_path)
    add_plannable_robot(robots,name = 'robot2', start_pos='robot2_start_pos',goal_pos = "robot2_goal_pos",robot_urdf_path = robot_urdf_path)
    blender_file_path = bpy.data.filepath
    json_dict = {
        "plannable_robots":robots,
    ...
    with open('C:/Users/kerim/Desktop/git/Manipulator-Dynamic-Planning/Blender/scenes'+'\\scene_task_multiagent.json', 'w') as f:
